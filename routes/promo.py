...
# ── Route publique : vérifier un code ────────────────────────
...
def list_promos(
    ...
    for p in promos:
        ...
        result.append({
            ...
            "reduction_fcfa": p.reduction_fcfa,
            "gain_influenceur": p.gain_influenceur,
            "quota": p.quota,
            ...
            "commandes": [{"ref": c.ref, "statut": c.statut, "created_at": str(c.created_at)} for c in cmds],
...
def update_promo(
    ...
    if "actif" in body:
        promo.actif = bool(body["actif"])
    if "quota" in body:
        promo.quota = int(body["quota"])
    if "reduction_fcfa" in body:
        promo.reduction_fcfa = float(body["reduction_fcfa"])
    if "gain_influenceur" in body:
        promo.gain_influenceur = float(body["gain_influenceur"])
    db.commit()
    return {"ok": True}
