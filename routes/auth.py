...
def hash_password(plain: str) -> str:
    ...
    Fallback : retourne le texte brut si bcrypt non installé.
    """
    if not _bcrypt_available():
        print("[auth] ⚠️  bcrypt non disponible — mot de passe stocké en clair")
        return plain
    import bcrypt
...
def verify_password(plain: str, stored: str) -> bool:
    ...
    Supporte la transition : ancien mot de passe en clair → comparaison directe.
    Nouveau mot de passe hashé → vérification bcrypt.
    """
    if not stored:
        return False
    if not stored.startswith("$2b$") and not stored.startswith("$2a$"):
        return plain == stored  # ancien mot de passe en clair
    if not _bcrypt_available():
        return plain == stored
    import bcrypt
    try:
...
def migrate_pwd_to_bcrypt(db: Session):
    """
    Migration progressive au startup :
    hashe tous les mots de passe encore en clair (patron + employés).
    """
    try:
        cfg = db.query(Config).first()
        if cfg and cfg.admin_pwd and not cfg.admin_pwd.startswith("$2b$"):
            hashed = hash_password(cfg.admin_pwd)
            db.execute(
                text("UPDATE configs SET admin_pwd = :h WHERE id = :id"),
                {"h": hashed, "id": cfg.id}
            )
            print("✅ Mot de passe patron migré vers bcrypt")
    except Exception as e:
        print(f"[auth] Erreur migration patron: {e}")
        db.rollback()

    try:
        emps = db.query(Employe).filter(Employe.actif == True).all()
        n = 0
        for e in emps:
            if e.pwd and not e.pwd.startswith("$2b$") and not e.pwd.startswith("$2a$"):
                e.pwd = hash_password(e.pwd)
                n += 1
        if n:
            print(f"✅ {n} mot(s) de passe employé(s) migrés vers bcrypt")
    except Exception as e:
        print(f"[auth] Erreur migration employés: {e}")
        db.rollback()
...
# ══════════════════════════════════════════════════════════════
# MIDDLEWARES — définis avant les endpoints qui en dépendent
# ══════════════════════════════════════════════════════════════
...
def login(body: Dict[str, Any], response: Response, request: Request,
    ...
    # Patron
    if cfg.admin_pwd and verify_password(password, cfg.admin_pwd):
        role = "patron"
        # Migration en ligne si encore en clair
        if not cfg.admin_pwd.startswith("$2b$") and not cfg.admin_pwd.startswith("$2a$"):
            try:
                cfg.admin_pwd = hash_password(password)
                db.commit()
            except Exception:
                db.rollback()

    else:
        # Employés — itération nécessaire car verify_password ne peut pas être dans un filtre SQL
        emps = db.query(Employe).filter(Employe.actif == True).all()
        for emp in emps:
            if verify_password(password, emp.pwd or ""):
                role = getattr(emp, "role", None) or "employe"
                if role not in ("employe", "logisticien"):
                    role = "employe"
                # Migration en ligne si encore en clair
                if emp.pwd and not emp.pwd.startswith("$2b$") and not emp.pwd.startswith("$2a$"):
                    try:
                        emp.pwd = hash_password(password)
                        db.commit()
                    except Exception:
                        db.rollback()
                break
    ...
    if not role:
        ...
        print(f"🚨 Tentative login échouée — IP: {ip}")
        _time.sleep(0.5)
...
def reset_password(body: Dict[str, Any], request: Request, db: Session = Depends(get_db)):
    ...
    if not new_password or len(new_password) < PWD_MIN_LENGTH:
        raise HTTPException(400, f"Mot de passe trop court (minimum {PWD_MIN_LENGTH} caractères)")
...
def totp_setup(request: Request, db: Session = Depends(get_db),
    ...
    if not _totp_available():
        raise HTTPException(500, "pyotp non installé")
    cfg    = get_config(db)
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(500, "Erreur sauvegarde secret")
    qr_base64 = None
    try:
        ...
        pass
    return {"secret": secret, "uri": uri, "qr_base64": qr_base64}
...
def totp_confirm(body: Dict[str, Any], request: Request,
    ...
    if not secret:
        raise HTTPException(400, "Aucun secret TOTP — lancez d'abord /totp/setup")
...
def totp_disable(body: Dict[str, Any], request: Request,
    ...
    password = str(body.get("password", "")).strip()
    cfg      = get_config(db)
    if not password or not verify_password(password, cfg.admin_pwd or ""):
